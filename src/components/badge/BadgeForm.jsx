...
const BadgeForm = () => {
    ...
    const handleFileSelect = (e) => {
        ...
        if (file.size > 5 * 1024 * 1024) {
            setModalMessage('이미지 파일은 5MB 이하여야 합니다.');
            setModalType('error');
            setShowModal(true);
            return;
        }
        ...
        if (!file.type.startsWith('image/')) {
            setModalMessage('이미지 파일만 업로드 가능합니다.');
            setModalType('error');
            setShowModal(true);
            return;
        }
    ...
    const handleImageUpload = async () => {
        if (!selectedFile) {
            setModalMessage('이미지 파일을 선택해주세요.');
            setModalType('error');
            setShowModal(true);
            return;
        }
        ...
        try {
            ...
            setModalMessage(
                '이미지 업로드 중 오류가 발생했습니다: ' + err.message
            );
            setModalType('error');
            setShowModal(true);
    ...
    const handleSubmit = async (e) => {
        ...
        if (!name || !icon || !category || !requirement) {
            setModalMessage('비어있는 칸이 있습니다. 칸을 모두 채워주세요.');
            setModalType('error');
            setShowModal(true);
            return;
        }
        ...
        if (isNaN(requirementNum) || requirementNum <= 0) {
            setModalMessage('획득 기준은 양수여야 합니다.');
            setModalType('error');
            setShowModal(true);
            return;
        }
        ...
        try {
            ...
            setModalMessage('뱃지가 성공적으로 등록되었습니다!');
            setModalType('success');
            setShowModal(true);
            // 폼 초기화
            setName('');
            ...
            let errorMessage = '뱃지 추가 중 오류가 발생했습니다.';
            ...
            <div className='bg-white shadow-md rounded-lg p-6 space-y-4'>
                ...
                <div className='bg-blue-50 border border-blue-200 rounded-md p-4 text-sm mt-6'>
                    ...
                    <ul className='space-y-1 text-blue-800'>
                        ...
                        <li>
                            • <strong>재활용센터</strong>: 재활용센터 방문 관련
                            뱃지
                        </li>
                        <li>
