...
export default function EditProfileScreen({ onBack, onNavigate }) {
    const navigate = useNavigate();
    ...
    const stopbeingmember = async () => {
        try {
            ...
            // Redux 상태 초기화 (로그아웃 액션 사용 - token, memberId 삭제 및 상태 초기화)
            dispatch(logout());

            // 모든 스토리지 정리 (회원탈퇴 시 모든 데이터 완전 삭제)
            // 단, 회원탈퇴 플래그는 새로고침 후에도 유지되도록 localStorage에 저장
            const accountDeactivated = 'accountDeactivated';
            localStorage.clear();
            sessionStorage.clear();

            // 회원탈퇴 완료 플래그 설정 (홈 화면 및 App.jsx에서 리다이렉트 방지용)
            ...
            // 2초 후 홈으로 이동 (페이지 강제 새로고침으로 모든 상태 초기화)
            setTimeout(() => {
                // 모달 닫기
                setModal(null);

                // 페이지를 완전히 새로고침하여 홈으로 이동
                // 이를 통해 모든 상태가 완전히 초기화되고 리다이렉트 문제 방지
    ...
    return (
        <div className='auth-wrap'>
            ...
            <div className='card'>
                ...
                {/* 닉네임 */}
                <div className='field'>
                    <label className={`label ${nickname ? 'filled' : ''}`}>
                        닉네임
                    </label>
                    <input
                        className={`input ${
                ...
                {/* 상태 메시지 */}
                {
                    ...
                    nicknameValid && nickAvailable === true ? (
                        <span style={{ color: '#3fa14a' }}>
                            사용 가능한 닉네임입니다
                        </span>
                    ) : nicknameValid && nickAvailable === false ? (
                        <span style={{ color: '#d33b3b' }}>
                            이미 존재하는 닉네임입니다
                        </span>
                    ) : null
                }
