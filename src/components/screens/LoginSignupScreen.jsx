    ...
    <div className="auth-wrap">
      ...
      <div className="card">
        ...
        {!userInfo && (
          <button 
  onClick={kakaoLogin} 
  style={{ width: "100%", marginTop: "12px", borderRadius: "12px", overflow: "hidden", padding: 0 }}
>
  <img 
    src={kakaoBtn} 
    alt="카카오 로그인"
...
function LoginForm({ setUserInfo }) {
  ...
  const submitLogin = async () => {
    try {
      ...
      const info = await axios.get("http://localhost:8080/member/me", {
...
function SignupForm({ setPage }) {
  ...
  const formValid =
    emailValid && pwValid && confirmValid && nicknameValid &&
    emailAvailable === false && nickAvailable === false;
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        ...
        const res = await axios.get("http://localhost:8080/member/check-email", {
  params: { email }
});
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        const res = await axios.get("http://localhost:8080/member/check-nickname", {
  params: { nickname }
});
  ...
  return (
    <form onSubmit={(e) => e.preventDefault()}>
      ...
      />
      {emailValid && emailAvailable === false && <span style={{color:"green"}}>사용 가능한 이메일입니다</span>}
      {emailValid && emailAvailable === true && <span style={{color:"red"}}>이미 등록된 이메일입니다</span>}
      ...
      />
      {nicknameValid && nickAvailable === false && <span style={{color:"green"}}>사용 가능한 닉네임입니다</span>}
      {nicknameValid && nickAvailable === true && <span style={{color:"red"}}>이미 존재하는 닉네임입니다</span>}
  ...
  return (
    <div className="field">
      ...
      {showInvalid ? (
        ...
        <div className="hint">{type === "password" ? <p> </p> : <p> </p>}</div>
      )}
    </div>
  );
}
